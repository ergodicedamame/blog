...
What we are after the "optimal" ordering, i.e. the re-ordering with similar values placed close to each other.
...
Generally however, indices will be randomly assigned, in which case there is never a good reason to order by the original indices when plotting.
