---
title: "Generative vs. discriminative models: what's the difference really?"
subtitle: "Thinking through the differences between generative and discriminative models."
date: "2022/04/03"
image: "params.png"
categories: [factor graphs, ML, theory]
echo: false
jupyter:
  jupytext:
    text_representation:
...
Looking around, there seem to be multiple definitions of what discriminative and generative models actually are, ranging from too narrow and concrete to outright confusing, as well as lots of claims about advantages and disadvantages of one over the other, but often with little detail or justification. 
...
In order to do so, we will focus on _classification_, where we are given a labelled data set $D = \lbrace \left( x^i, y^i \right) \rbrace_N$ with inputs $x^i = \left( x^i_1, \cdots, x^i_n \right)$ and labels $y^i \in \lbrace 0, 1 \rbrace$, and we want $P \left( y = 1 \vert x = \hat x \right)$ for some (new) $\hat x$[^2].
...
This can be represented as a factor graph.
...
The more general definition of a generative model, and the one that we will use is of a model that can generate data via _ancestral sampling_, i.e. sampling from the priors and passing the sampled values through the graphical model.
...
In order to distinguish between the generative model and the extended discriminative model, which is also a generative model, Mitchell refers to the former as a _Bayes classifier_ given that it uses Bayes theorem to recover $P \left( y \vert x \right)$ [@mitchell2020generative].
...
Despite devoting a lot of pages to the discriminative and generative approaches, @bishop2006pattern also uses this definition in Section 8.1.2, albeit in passing.
Similarly, it is not uncommon to hear people refer to the models considered in McElreath's Statistical Rethinking [-@mcelreath2020statistical] as generative models, given all the space devoted to the causal DAGs and data generation, even though the models are actually discriminative regressions. 
...
We therefore distinguish between generative models and processes.
...
Let's write the generative model with parameters as
$$
...
Source(model_generative_params_str)
```
...
In order to compare it with the generative model, we expand the discriminative model with a probability over $x$ and a second parameter in order to obtain the joint distribution
$$
...
\end{align*}
$$
where we have used the iid assumption on the data, and then inferring $P \left( \theta, \phi \vert D \right)$.
...
$$
then we see that Bayes' update rule simplifies, namely
$$
\begin{align*}
...
We can now see that the fundamental difference between the two models is down to the discriminative one considering a larger parameter space without the constraint $\theta = \phi$.
...
In order to understand why generative models require more modelling assumptions to make inference manageable, we will consider the case of Boolean inputs $x = \left( x_1, \cdots, x_n \right)$, $x_j \in \lbrace 0, 1 \rbrace$.
...
Let us now look at the parameters necessary for the generative model by first considering $P\left( x \vert y \right)$ represented as a conditional probability table with $x$ represented as a single vector.
...
This is less than for the generative model. 
...
On top of the number of parameters to estimate, in order to reliably estimate the parameters, we need to observe each distinct instance multiple times.
...
Let's turn to the fact that generative models can deal with missing data, which is often mentioned.
...
\end{align*}
$$
because we assume that the past and future observations are conditionally independent given $\theta$.
...
Interestingly, this equates to making assumptions about the distribution $P \left( x \right)$, which we would instead have if we consider an extended discriminative model.
